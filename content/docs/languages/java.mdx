...
public final class Telemetry implements AutoCloseable {
  ...
  public Telemetry(String endpoint) {
    Resource resource = Resource.getDefault().merge(Resource.create(
        Attributes.of(ServiceAttributes.SERVICE_NAME, "checkout-java")));
...
`BatchSpanProcessor` gom span trước khi export để giảm network overhead. Import
`ServiceAttributes` từ semantic conventions artifact tương thích với BOM; nếu
API semantic conventions đang thay đổi trong release của bạn, dùng key chuẩn
`AttributeKey.stringKey("service.name")` thay vì API đã deprecated.
...
openTelemetry.getPropagators().getTextMapPropagator().inject(
    context, requestBuilder, (builder, key, value) -> builder.header(key, value));
```
